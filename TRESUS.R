...
PTLCTS = merge(dfnewhistogram,dfTSS1)
PTLCTS=na.omit(PTLCTS)
head(PTLCTS)
unique(PTLCTS$PropAg)
plot(PTLCTS$PopTrend~PTLCTS$PropAg)
x=lm(formula = PTLCTS$PopTrend~PTLCTS$PropAg)
abline(x)
abline(a=0,b=0)
summary(x)
dfPTLCTS=as.data.frame(PTLCTS)
write.csv(dfPTLCTS,"PTLCTS.csv",FALSE)
dfPTLCTS=read.csv('PTLCTS.csv',header=T)

##Model Suite##

#Null Model: Population Trend Varies Randomly (not varying with propag or coordinate)
lme1 <- lme(PopTrend ~ 1, random=list(~1|Observer_Number), data=dfPTLCTS)
#Population trend varies with relation to proportion agriculture
lme2 <- lme(PopTrend ~ PropAg, random=list(~1|Observer_Number), data=dfPTLCTS)
...
lme3 <- lme(PopTrend ~ PropAg + Longitude, random=list(~1|Observer_Number), data=dfPTLCTS)
...
lme4 <- lme(PopTrend ~ PropAg + Latitude, random=list(~1|Observer_Number), data = dfPTLCTS)
#Population trend varies with relation to proportion agriculture, latitude and longitude (reasons above)
lme5 <- lme(PopTrend ~ PropAg + Longitude + Latitude, random=list(~1|Observer_Number), data=dfPTLCTS)
